...
class TFFHandler(ApproximatorHandler):
    ...
    def calibrate(self, pricer_template, tff_input_raw_factor_names, tff_input_raw_base_values,
        ...
        product_static = create_product_static_from_dict(product_static_params_for_worker)
...
@approximator_handler("FULL")
class FullPricerHandler(ApproximatorHandler):
    """Pass-through handler for full revaluation (no model fitting)."""

    def get_approximator_type(self) -> str:
        return "FULL"

    def create_calibrator(self, *args, **kwargs):
        return None
    ...
    def calibrate(self, pricer_template, tff_input_raw_factor_names, tff_input_raw_base_values,
                  product_static_params_for_worker, pricer_config_for_worker, actual_rate_pillars,
                  scenarios_for_this_approximator, config_params):
        # Return a no-op calibration result to keep interface parity
        return {
            'model_dict': {},
            'raw_input_names': list(tff_input_raw_factor_names) if tff_input_raw_factor_names is not None else [],
            'normalization_params': {},
            'option_feature_order': config_params.get('option_feature_order', 0) if isinstance(config_params, dict) else 0,
            'rmse': 0.0,
            'fit_time_seconds': 0.0,
            'base_value': float(tff_input_raw_base_values[0]) if getattr(tff_input_raw_base_values, 'size', 0) else 0.0,
            'base_approximator_value': 0.0,
            'fixed_pricer_params': {}
        }
