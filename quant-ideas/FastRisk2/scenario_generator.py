...
class SimpleRandomScenarioGenerator(ScenarioGeneratorBase):
    def __init__(self,
                 ...
                 # Inflation factors (simple additive rates)
                 base_inflation_map: dict[str, float] = None, # e.g., {"USD_INFLATION": 0.02}
                 inflation_shock_std_dev_map: dict[str, float] = None,
        ...
        self.base_inflation_map = base_inflation_map if base_inflation_map is not None else {}
        self.inflation_shock_std_dev_map = inflation_shock_std_dev_map if inflation_shock_std_dev_map is not None else {}
        ...
        self._configured_factor_names_ordered = sorted(list(
            ...
            set(self.base_inflation_map.keys())
        ))
    ...
    def generate_scenarios(self, num_scenarios: int, target_factor_names: list[str] = None) -> tuple[np.ndarray, list[str]]:
        ...
        for factor_name in factors_to_generate:
            ...
            elif factor_name in self.base_inflation_map:
                shock_std = self.inflation_shock_std_dev_map.get(factor_name, 0.0005)
                shocks = self.rng.normal(loc=0.0, scale=shock_std, size=num_scenarios)
                shocks[0] = 0.0
                factor_column = base_value + shocks
                factor_column = np.maximum(factor_column, -0.01)  # avoid highly negative inflation
