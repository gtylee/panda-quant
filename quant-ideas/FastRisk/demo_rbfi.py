...
# Core imports
from workflow_manager import Portfolio, InstrumentProcessor, PortfolioBuilder, TFFConfigurationFactory, generate_portfolio_specs_for_serialization, generate_price_strips, reconstruct_product_static
...
def run_callable_bond_rbfi_demo(
    ...
    for bond_id, tff_model in tff_model_registry.items():
        ...
        product_static = reconstruct_product_static(tff_instance['product_static_dict'])
        ...
        try:
           ...
           # print(f"  {bond_id}: RBFI fitted, RMSE={rmse:.6f}, base_diff={abs(base_val-base_rbfi_val):.4f}")
    ...
    for bond_id, rbfi_model in rbfi_results.items():
        try:
            ...
            if norm_params.get('is_engineered', False):
                # For simplicity, assume raw features for this demo
                # In full implementation, you'd apply the same feature engineering here
                pass
            ...
            # If one bond fails, continue with others
            continue
